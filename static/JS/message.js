...
function append_message(message) {
    ...
    if (message['username'] == localStorage.getItem('username')) {
        // Message with X icon
        this_message.innerHTML = `<p><b>${message['username']}</b> @ ${time}: ${message['message']}<i class="fas fa-times-circle" onclick="delete_message()"></i></p>`;
        this_message.classList.add('sent');
    }
    else if (message['username'] == 'Admin') {
        this_message.innerHTML = `<p><b>${message['username']}</b> @ ${time}: ${message['message']}</p>`;
    ...
    document.querySelector('#channel_messages').appendChild(this_message).scrollIntoView({behavior: "smooth"});
}
